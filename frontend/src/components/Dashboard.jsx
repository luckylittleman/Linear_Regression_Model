...
const getRisk = (score) => {
  if (score < 40) return { label: 'High Risk',     color: '#f87171', bg: 'rgba(248,113,113,0.15)' };
  if (score < 60) return { label: 'Moderate Risk', color: '#fbbf24', bg: 'rgba(251,191,36,0.15)'  };
  return              { label: 'Safe',             color: '#34d399', bg: 'rgba(52,211,153,0.15)'  };
};
...
const Dashboard = () => {
  ...
  const downloadFullReport = () => {
    ...
    if (!dataToExport.length) { alert('No data to export. Process a batch first.'); return; }
    const headers = 'Student Name,Registration No,Predicted Grade (%),Risk Category\n';
    const rows = dataToExport.map(s =>
      `${s.student_name || s.name},${s.reg_no},${s.predicted_score}%,${s.risk_category || getRisk(s.predicted_score).label}`
    ).join('\n');
    const blob = new Blob([headers + rows], { type: 'text/csv' });
    const url  = window.URL.createObjectURL(blob);
    const a    = document.createElement('a');
    a.href = url; a.download = `CS_Report_${new Date().toISOString().split('T')[0]}.csv`;
    ...
    <div style={{ animation: 'fadeIn 0.5s ease-in' }}>
      ...
      <div className="card" style={{ padding: '60px', textAlign: 'center', color: '#a1a1aa' }}>
        ...
        <p style={{ margin: 0, fontSize: '0.9rem' }}>
          Upload a student CSV from the <strong style={{ color: '#2dd4bf' }}>Batch Upload</strong> tab to see analytics here.
        </p>
      </div>
    </div>
  );
    ...
    <div style={{ animation: 'fadeIn 0.5s ease-in' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '16px', marginBottom: '30px' }}>
        <div className="card" style={statCardStyle}>
          ...
          <h2 style={{ color: '#2dd4bf', margin: '10px 0 0' }}>{analytics.total}</h2>
        </div>
        <div className="card" style={statCardStyle}>
          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
            <p style={labelStyle}>PREDICTED MEAN</p><Target size={16} color="#fbbf24" />
          </div>
          <h2 style={{ color: '#fbbf24', margin: '10px 0 0' }}>{analytics.mean}%</h2>
        </div>
        <div className="card" style={statCardStyle}>
          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
            <p style={labelStyle}>🔴 HIGH RISK (&lt;40%)</p><Activity size={16} color="#f87171" />
          </div>
          <h2 style={{ color: '#f87171', margin: '10px 0 0' }}>{analytics.atRisk}</h2>
        </div>
        <div className="card" style={statCardStyle}>
          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
            <p style={labelStyle}>🟢 SAFE (≥60%)</p><Activity size={16} color="#34d399" />
          </div>
          <h2 style={{ color: '#34d399', margin: '10px 0 0' }}>{analytics.safe ?? '—'}</h2>
        </div>
      </div>

      {/* Chart — 3-band traffic light */}
      <div className="card" style={{ height: '380px', background: '#0c0d21', marginBottom: '25px' }}>
        <h3 style={{ marginBottom: '20px', fontSize: '1rem', color: '#e2e8f0' }}>
          Grade Distribution — Traffic Light Risk Bands
        </h3>
        <ResponsiveContainer width="100%" height="82%">
          <BarChart data={analytics.chartData}>
            <CartesianGrid strokeDasharray="3 3" stroke="#1e1e3f" vertical={false} />
            <XAxis dataKey="range" stroke="#a1a1aa" fontSize={11} tickLine={false} axisLine={false} />
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                ...
                <tbody>
                  {filtered.length > 0 ? filtered.map((s, i) => {
                    const rc = s.risk_category || getRisk(s.predicted_score).label;
                    const { color, bg } = getRisk(s.predicted_score);
                    return (
                      <tr key={i} style={{ borderBottom: '1px solid #151630' }}>
                        ...
                        <td style={tdStyle}>
                          <span style={{
                            ...
                            background: bg, color, textTransform: 'uppercase', border: `1px solid ${color}44`
                          }}>
                            {rc}
                          </span>
                        </td>
                    ...
                    <tr><td colSpan="4" style={{ padding: '30px', textAlign: 'center', color: '#a1a1aa' }}>No results match your search.</td></tr>
                  )}
                </tbody>
              </table>
            </div>
...
const downloadBtnStyle = { display: 'flex', alignItems: 'center', gap: '8px', padding: '10px 20px', background: 'transparent', border: '1px solid #2dd4bf', color: '#2dd4bf', borderRadius: '6px', cursor: 'pointer', fontWeight: 'bold', fontSize: '0.85rem', flexShrink: 0 };
const clearBtnStyle    = { display: 'flex', alignItems: 'center', gap: '8px', padding: '10px 18px', background: 'transparent', border: '1px solid #f87171', color: '#f87171', borderRadius: '6px', cursor: 'pointer', fontWeight: 'bold', fontSize: '0.85rem', flexShrink: 0 };
